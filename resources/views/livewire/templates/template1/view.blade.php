...
<div class="max-w-7xl mx-auto bg-white shadow-lg rounded-2xl overflow-hidden p-6 space-y-8">
    {{-- زر الرجوع --}}
    <a href="{{ route("template",$id) }}" wire:navigate class="inline-flex items-center text-sm text-orange-600 hover:underline">
        ← الرجوع
    </a>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 items-start">
        ...
        <div class="space-y-4">
            ...
            {{-- وصف الخدمة --}}
            <div class="text-sm text-slate-600 leading-relaxed max-h-48 overflow-y-auto border-t pt-4 whitespace-pre-line">
                {!! nl2br(e($offering->description)) !!}
            </div>

            {{-- التاجر --}}
            @if ($offering->user)
            <a href="{{ route('template', $id) }}"
            wire:navigate
