<div class="mb-6 font-sans">
    <label class="block text-2xl font-extrabold mb-6 text-gray-800">الوجهات السياحية</label>

    <div class="flex space-x-6 overflow-x-auto pb-4">
        @foreach($Destenations as $destination)
            <div x-data="{ open: false }" class="flex-shrink-0 w-64 bg-white rounded-2xl shadow-md hover:shadow-xl transition cursor-pointer overflow-hidden">
                ...
                <div class="h-40 w-full overflow-hidden relative">
                    @if(!empty($destination['image']))
                        <img src="{{ asset('storage/' . $destination['image']) }}" alt="صورة الوجهة" class="object-cover w-full h-full transition-transform duration-300 hover:scale-105"/>
                    @else
                        <div class="h-full w-full flex items-center justify-center bg-gray-100">
                            <i class="ri-map-pin-line text-6xl text-gray-400"></i>
                        </div>
                    @endif
                </div>
                ...
                <div class="p-4 space-y-2">
                    <h3 class="text-lg font-bold text-gray-800 truncate">{{ $destination['name'] ?? 'بدون اسم' }}</h3>
                    <p class="text-sm text-gray-600 line-clamp-3">{{ $destination['description'] ?? 'لا يوجد وصف' }}</p>
                ...
                <div x-show="open" x-cloak class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
                    <div class="bg-white rounded-2xl w-full max-w-md p-6 relative shadow-xl">
                        ...
                        <!-- صورة الوجهة داخل المودال -->
                        @if(!empty($destination['image']))
                            <div class="h-48 w-full mb-4 overflow-hidden rounded-xl">
                                <img src="{{ asset('storage/' . $destination['image']) }}" alt="صورة الوجهة" class="object-cover w-full h-full"/>
                            </div>
                        @endif
