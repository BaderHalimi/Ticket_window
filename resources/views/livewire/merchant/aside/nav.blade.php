<nav class="flex flex-col gap-2">
    ...
    <div x-data="{ show: false, msg: '' }"         class="relative group cursor-not-allowed select-none "
>
    <!-- Toast -->
    <div 
        x-show="show" 
        x-transition 
        x-text="msg"
        class="fixed top-5 right-5 bg-red-600 text-white px-4 py-2 rounded-lg shadow-lg z-50"
        @click="show = false"
    ></div>

    <fieldset 
        disabled 
        class="relative group cursor-not-allowed select-none "
        @click="show = true; msg = 'لم توافق الإدارة عنك بعد'" 
        @touchstart="show = true; msg = 'لم توافق الإدارة عنك بعد'"
    >
