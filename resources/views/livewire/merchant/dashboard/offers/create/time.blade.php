        ...
        @if ($offering->type == 'services')
            <div class="border rounded-md p-4 space-y-4">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4 items-center">
                    <label class="flex items-center gap-2">
                        <input type="checkbox" wire:model="default_day.enabled" class="form-checkbox">
                        <span>مفعل</span>
                    </label>
                    <input type="time" wire:model="default_day.from" class="border rounded p-2">
                    <input type="time" wire:model="default_day.to" class="border rounded p-2">
                    ...
                    @foreach(['saturday','sunday','monday','tuesday','wednesday','thursday','friday'] as $dayName)
                        <div class="border rounded-md p-3">
                            <label class="flex items-center gap-2 mb-2">
                                <input type="checkbox" wire:model="day.{{ $dayName }}" class="form-checkbox">
                            ...
                            @if(!empty($day[$dayName]))
                                <div class="space-y-2">
                                    <div>
                                        ...
                                        <input type="time" wire:model="from_time.{{ $dayName }}" class="w-full border rounded-md p-2">
                                    </div>
                                    <div>
                                        <label class="block text-sm mb-1">إلى:</label>
                                        <input type="time" wire:model="to_time.{{ $dayName }}" class="w-full border rounded-md p-2">
                                    </div>
                                </div>
                            @endif
                        </div>
                    ...
                    <div class="border rounded p-3 space-y-2">
                        <div class="grid grid-cols-2 gap-2">
                            <div>
                                ...
                                <input type="date" wire:model="calendar.{{ $index }}.start_date" class="w-full border rounded-md p-2">
                            </div>
                            <div>
                                <label class="block text-sm mb-1">إلى تاريخ:</label>
                                <input type="date" wire:model="calendar.{{ $index }}.end_date" class="w-full border rounded-md p-2">
                            </div>
                        </div>
                        <div class="grid grid-cols-2 gap-2">
                            <div>
                                <label class="block text-sm mb-1">من وقت:</label>
                                <input type="time" wire:model="calendar.{{ $index }}.start_time" class="w-full border rounded-md p-2">
                            </div>
                            <div>
                                <label class="block text-sm mb-1">إلى وقت:</label>
                                <input type="time" wire:model="calendar.{{ $index }}.end_time" class="w-full border rounded-md p-2">
                            </div>
                        </div>
