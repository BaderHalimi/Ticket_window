    ...
    <form class="space-y-8">
        ...
        <div class="bg-white rounded-xl p-6 shadow-md border border-gray-100">
            ...
            <div class="relative">
                ...
                <input type="number" step="0.01" min="0" max="999999.99" wire:model.lazy="base_price"
        ...
        <div class="bg-white rounded-xl p-6 shadow-md border border-gray-100">
            <div class="flex items-center justify-between mb-6">
                ...
                <label class="relative inline-flex items-center cursor-pointer">
                    ...
                    <div
                        class="w-11 h-6 bg-gray-200 peer-focus:outline-none rounded-full peer peer-checked:bg-indigo-600 transition-all">
                    </div>
                    <div
                        class="absolute left-0.5 top-0.5 w-5 h-5 bg-white rounded-full shadow-md transform peer-checked:translate-x-full transition-all">
                    </div>
                </label>
            </div>
            ...
            @if ($enable_coupons)
                <div class="space-y-6">
                    ...
                    @if (count($coupons) > 0)
                        <div class="space-y-4">
                            ...
                            @foreach ($coupons as $index => $coupon)
                                <div class="bg-gradient-to-r from-indigo-50 to-purple-50 rounded-xl p-6 border border-indigo-200 shadow-sm"
                                    wire:key="coupon-{{ $index }}">
                                    <div class="flex items-center justify-between mb-4">
                                        <div class="flex items-center">
                                            ...
                                            <div class="ml-3">
                                                <h5 class="font-bold text-gray-800">{{ __('Coupon') }}
                                                    #{{ $index + 1 }}</h5>
                                                <p class="text-xs text-gray-600">{{ __('Promotional discount code') }}
                                                </p>
                                            </div>
                                        </div>
                                        <button type="button" wire:click="removeCoupon({{ $index }})"
                                    ...
                                    @if (isset($coupon['code']) && isset($coupon['discount']) && isset($coupon['type']))
                                        <div
                                            class="mt-6 p-4 bg-white rounded-lg border-2 border-dashed border-indigo-300">
                                            <div class="flex items-center justify-between">
                                                <div class="flex items-center">
                                                    <div
                                                        class="bg-indigo-600 text-white px-3 py-1 rounded-full text-sm font-bold">
                                                        {{ $coupon['code'] }}
                                                    </div>
                                                    <div class="ml-3">
                    ...
                    <div class="flex justify-center pt-4 border-t border-gray-200">
                        <button type="button" wire:click="addCoupon"
        ...
        <div class="bg-white rounded-xl p-6 shadow-md border border-gray-100">
            <div class="flex items-center justify-between mb-6">
                <div class="flex items-center">
                    ...
                    <div>
                        ...
                        <p class="text-sm text-gray-600">
                            {{ __('Automatically apply a discount during a specific period') }}</p>
                    </div>
                </div>
                <label class="relative inline-flex items-center cursor-pointer toggle-switch">
                    ...
                    <div
                        class="w-12 h-7 bg-gray-200 peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-emerald-300 rounded-full peer peer-checked:bg-gradient-to-r peer-checked:from-emerald-600 peer-checked:to-green-600 transition-all duration-300">
                    </div>
                    <div
                        class="absolute left-1 top-1 w-5 h-5 bg-white rounded-full shadow-lg transform peer-checked:translate-x-5 transition-all duration-300 flex items-center justify-center">
                        <i
                            class="ri-check-line text-xs text-emerald-600 opacity-0 peer-checked:opacity-100 transition-opacity duration-200"></i>
                    </div>
                </label>
            </div>
            ...
            @if ($enable_discounts)
                <div class="space-y-6">
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                        ...
                        <div>
                            ...
                            <div class="relative">
                                ...
                                <input type="datetime-local" wire:model.lazy="discount_start"
                        ...
                        <div>
                            ...
                            <div class="relative">
                                ...
                                <input type="datetime-local" wire:model.lazy="discount_end"
                    ...
                    @if ($discount_start && $discount_end && $discount_percent && $base_price)
                        ...
                        @if ($showPreview)
                            <div
                                class="bg-gradient-to-r from-emerald-50 to-green-50 rounded-xl p-6 border-2 border-emerald-200 shadow-sm">
                                <div class="flex items-center justify-between">
                                    <div>
                                        ...
                                        <div class="space-y-2">
                                            <div class="flex items-center text-sm text-emerald-700">
                                                ...
                                                <span>{{ __('Active from') }}
                                                    {{ $startDate->format('M d, Y H:i') }}</span>
                                            </div>
                                            <div class="flex items-center text-sm text-emerald-700">
                                                <i class="ri-calendar-check-line mr-2"></i>
                                                <span>{{ __('Until') }}
                                                    {{ $endDate->format('M d, Y H:i') }}</span>
                                            </div>
                                            <div class="flex items-center text-sm text-emerald-700">
                                                <i class="ri-time-line mr-2"></i>
                                                <span>{{ __('Duration:') }}
                                                    {{ $startDate->diffForHumans($endDate, true) }}</span>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="text-right">
                                        <div class="bg-white rounded-lg p-4 shadow-sm border border-emerald-200">
                                            ...
                                            <p class="text-sm text-gray-500 line-through">
                                                {{ number_format($base_price, 2) }} {{ __('SAR') }}</p>
                                            <p class="text-xs text-emerald-600 font-medium">{{ $discount_percent }}%
                                                {{ __('OFF') }}</p>
                                            <p class="text-lg font-bold text-emerald-600">
                                                {{ number_format($base_price - ($base_price * $discount_percent) / 100, 2) }}
                                                {{ __('SAR') }}
                                            </p>
                                        </div>
                                    </div>
                        ...
                        <div class="flex items-start">
                            ...
                            <div class="text-sm text-emerald-700">
                                ...
                                <ul class="space-y-1 text-xs">
                                    ...
                                    <li>• {{ __('Consider your profit margins when setting discount percentages') }}
                                    </li>
                                </ul>
                            </div>
                        </div>
            ...
            @else
                <div class="text-center py-8 text-gray-500">
                    ...
                    <p class="text-sm">
                        {{ __('Enable scheduled discounts to create time-limited promotional offers') }}</p>
                </div>
            @endif
        </div>
        ...
        <div>
            <div class="flex items-center justify-between">
                ...
                <label class="relative inline-flex items-center cursor-pointer">
                    ...
                    <div
                        class="absolute left-0.5 top-0.5 w-5 h-5 bg-white rounded-full shadow transform peer-checked:translate-x-full transition-all">
                    </div>
                </label>
            </div>
            @if ($enable_cancellation)
                <div class="mt-2">
                    ...
                    <input type="number" step="0.01" wire:model.lazy="cancellation_fee"
                        class="w-full border rounded-md p-2">
                </div>
                <label class="text-sm font-medium">تحديد آخر وقت للإلغاء؟</label>

                <div class="mt-2">
                    <input type="number" wire:model.lazy="cancellation_deadline_minutes"
                        placeholder="عدد الدقائق قبل البدء" class="w-full border rounded-md p-2">
                </div>
            @endif
                ...
                <div class="mt-4 space-y-3">
                    @foreach ($pricing_packages as $index => $pkg)
                        <div class="flex items-center gap-2">
                            <input type="text" wire:model.lazy="pricing_packages.{{ $index }}.label"
                                placeholder="مثال: شخص واحد" class="flex-1 border p-2 rounded-md">
                            <input type="number" step="0.01"
                                wire:model.lazy="pricing_packages.{{ $index }}.price" placeholder="السعر"
                                class="w-32 border p-2 rounded-md">
                            <button type="button" wire:click="removePackage({{ $index }})"
                                class="text-red-500 hover:underline text-sm">حذف</button>
                        </div>
                    @endforeach
                    <button type="button" wire:click="addPackage" class="text-blue-600 hover:underline text-sm">+
                        إضافة باقة</button>
                </div>
            @endif
        </div>
