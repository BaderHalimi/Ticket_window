                    ...
                    <tbody>
                        @foreach($reservations as $reservation)
                            <tr>
                                <td class="p-4">
                                    ...
                                    <div class="flex items-center gap-3">
                                        <span class="relative flex h-10 w-10 shrink-0 overflow-hidden rounded-full">
                                            <span class="flex h-full w-full items-center justify-center rounded-full bg-muted">{{ $reservation->user->name }}</span>
                                        </span>
                                        <div>
                                ...
                                <td class="p-4">{{ $reservation->offering->service_name }}</td>
                                <td class="p-4">
                                    @if($reservation->status == 'active')
                                        <span class="px-2.5 py-1 text-xs font-semibold rounded-full bg-green-100 text-green-800">نشط</span>
                                    @elseif($reservation->status == 'cancelled')
                                        <span class="px-2.5 py-1 text-xs font-semibold rounded-full bg-red-100 text-red-800">ملغي</span>
                                    @else
                                ...
                                <td class="p-4 text-center">
                                    <a href="" class="text-blue-500 hover:underline">عرض التفاصيل</a>
                                </td>
                            </tr>
                        @endforeach
                    </tbody>
